...
<sub>DeepSeek Harness: developer preview — the port uses only stock dsh capabilities (native `~/.agents/skills` discovery, the Claude Code hook bridge for the nudge), so harness drift degrades a surface rather than crashing. Needs a DeepSeek API key. Skill discovery and boot are verified; a live keyed session is not yet — first-run reports welcome. Details in [INSTALL-DSH.md](INSTALL-DSH.md).</sub><br>
